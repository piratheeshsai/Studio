<template>
    <div class="min-vh-100 bg-black text-white">
        ...
        <!-- Navigation -->
        <div
            class="bg-secondary bg-opacity-10 border-bottom border-secondary py-3"
        >
...
const statCards = [
    {
        title: "Total Shoots",
        value: stats.value.totalShoots,
        icon: "📸",
        change: "+12%",
    },
    {
        title: "Total Images",
        value: stats.value.totalImages.toLocaleString(),
        icon: "🖼️",
        change: "+8.2%",
    },
    {
    ...
    },
    {
        ...
        value: stats.value.activeGalleries,
        icon: "👁️",
        change: "+15.3%",
    },
];
...
const filteredShoots = computed(() =>
    activeShootType.value === "All"
        ? recentShoots.value
        : recentShoots.value.filter(
              (shoot) => shoot.type === activeShootType.value
          )
);
...
function copyGalleryLink(client) {
    const link = `https://gallery.studio.com/${client
        .toLowerCase()
        .replace(/\s+/g, "-")}-2024`;
...
const websiteSettings = ref({
    ...
    heroMessage:
        "Welcome to our photography studio where every moment becomes a masterpiece.",
    aboutText:
        "We are passionate photographers dedicated to capturing your most precious moments.",
